...
public class GeometricSequenceGenerator implements Iterator<Integer> {
  ...
  public GeometricSequenceGenerator(int init, int ratio, int size) {
    ...
    this.INIT = init; // sequence intitial value
    this.RATIO = ratio; // the ratio between sequence values
    next = init; // initializes next to the first element in this geometric progression
    generatedCount = 0; // initial count is zero
  }
  ...
  public boolean hasNext() {
    //time complexity: O(1)
    return generatedCount < SIZE; // if count is less than size return true, else false
  }
  ...
  public Integer next() {
    ...
    next = current * RATIO; // set the next element (multiplies the current number by the ratio)
